...
bool alabu(vll a,v99 sum)
{
    if(sum&1)return false;
    sum/=2;
    v99 kot=sum;
    for(v99 i=a.size()-1;~i;i--)
    {
        if(a[i]<=sum)sum-=a[i];
    }
    for(auto i:a)if(i<=kot)kot-=i;
    return sum==0LL or kot==0;
}

void solve()
{
    /// code is here->
    v99 n,sum=0,r,e;read(n,r);
    vll a;
    for(v99 i=0;i<n;i++)
    {
        v99 x;read(x);
        if(i==0){e=x%r;sum+=e;continue;}
        if(x%r)a.pb(abs(x%r)),sum+=abs(x%r);
    }
    sort(sob(a));


    v99 kot=sum%r;
    if(alabu(a,kot) or alabu(a,sum)) print("YES");
    else print("NO");
